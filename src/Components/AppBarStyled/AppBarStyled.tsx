    ...
    MuiAppBar: {
      styleOverrides: {
        root: {
          ...
          zIndex: 1100,
          transition:
            "box-shadow 300ms cubic-bezier(0.4, 0, 0.2, 1) 0ms, background-color 300ms cubic-bezier(0.4, 0, 0.2, 1) 0ms",
...
function AppBarStyled({
  ...
  const notificationsSubscribe = useRef<Unsubscribe>();
  ...
  useEffect(() => {
    if (notificationsSubscribe.current) notificationsSubscribe.current();
    // const q = query(collection(db, TURNS_COLLECTION), where("date", ">", "CA"));
    notificationsSubscribe.current = onSnapshot(
      query(
        collection(app.firestore, NOTIFICATIONS_COLLECTION),
        orderBy("date", "desc"),
        limit(visiblesNotifications)
  ...
  }, [app.firestore, visiblesNotifications]);
  ...
  return (
    <ThemeProvider theme={scrolling ? scrollingTheme : onTopTheme}>
      <AppBar position="sticky" color="primary">
        <Toolbar>
          <IconButton
            ...
            onClick={() => setOpenDrawerMovile(true)}
            edge="start"
            color="inherit"
            aria-label="menu"
          >
          ...
          <IconButton
            ...
            onClick={() => setOpenDrawerDesktop(!openDrawerDesktop)}
            edge="start"
            color="inherit"
            aria-label="menu"
          >
      ...
      >
        {notifications
          .slice(0, visiblesNotifications)
          .map((notification, indexN) => {
            return (
              <NotificationItem
        ...
        {notifications.length === 0 ? (
          ...
          <MenuItem onClick={() => setVisiblesNotifications((p) => p + 5)}>
            Mostrar mas ...
          </MenuItem>
        ) : (
