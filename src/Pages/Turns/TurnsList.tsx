...
function TurnsList() {
  ...
  useEffect(() => {
    ...
    filterTurns().map((t) => {
      ...
      if (index != -1) dates[index].turns.push(t);
      else
        dates.push({
          date: getDate(t),
          turns: [t],
  ...
  function setDesdeHandle(value: string) {
    ...
    dateExtra.setMinutes(
      dateExtra.getMinutes() + dateExtra.getTimezoneOffset()
    );
  ...
  function handleSaveTurn(): void {
    ...
    if (dialogTurn.id)
      updateDoc(
        doc(app.firestore, TURNS_COLLECTION, dialogTurn.id).withConverter(
          turnConverter
        ),
        dialogTurn
      )
      ...
      addDoc(
        collection(app.firestore, TURNS_COLLECTION).withConverter(
          turnConverter
        ),
        dialogTurn
      )
    ...
    >
      ...
      <FormGroup>
        <FormControlLabel
          control={
            <Switch
              checked={viewReserveds}
              onChange={(e) => setViewReserveds((prev) => !prev)}
            />
          }
          label="Mostrar reservados"
        />
        <FormControlLabel
          control={
            <Switch
              checked={viewOpeneds}
              onChange={(e) => setViewOpeneds((prev) => !prev)}
            />
          }
          label="Mostrar los libres"
        />
      </FormGroup>
